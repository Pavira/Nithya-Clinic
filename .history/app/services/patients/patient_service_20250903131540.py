...
async def view_and_search_patients_service(
    ...
    try:
        # 1) Build base query with projection
        fields = [
        ...
        if search_type == "full_name" and search_value:
            ...
            total_query = (
                collection_ref_field.order_by("FullName")
                .start_at([search_value])
                .end_at([search_value + "\uf8ff"])
                .get()
            )
            total_docs = len(total_query)

        elif search_type == "phone_number" and search_value:
            query = collection_ref.where("PhoneNumber", "==", int(search_value))
            total_query = collection_ref_field.where(
            ...
            # Use pre-count only for default listing
        ...
        return {
            ...
            "total_count": total_docs,  # ✅ now reflects filtered total count
        }
