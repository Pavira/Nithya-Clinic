...
// async function checkkDuplicatePatient(fullName, phoneNumber, exclude_id) {
//   console.log("Checking for duplicate patient...");
//   showLoader();
//   try {
//     const token = localStorage.getItem("token");
//     const url = `/api/v1/patients/check_duplicate_patient?full_name=${encodeURIComponent(fullName)}&phone_number=${phoneNumber}&exclude_id=${exclude_id}`;
...
//   }
// }
async function checkDuplicatePatient(fullName, phoneNumber, exclude_id = null) {
  console.log("Checking for duplicate patient...");
  showLoader();
  try {
    const token = localStorage.getItem("token");
    ...
    if (response.ok) {
      // backend already returns True = duplicate, False = not duplicate
      return result.data;
    } else {
      Swal.fire({
        icon: "error",
    ...
    document.getElementById("updateButton").addEventListener("click", async (e) => {
        ...
        // ----------- Check Duplicate Patient -----------
        console.log("Checking for duplicate patient...");
        const isDuplicate = await checkDuplicatePatient(fullName, phoneNumber, exclude_id = window.pageParams?.patient_id);
        if (isDuplicate) {
          hideLoader();
          Swal.fire({
              icon: 'error',
...
if (dobString && ageField) {
  ...
  document.getElementById("fullName").addEventListener("blur", async function () {
    ...
    if (fullName && phoneNumber.length === 10) {  
      // showLoader();
      const isDuplicate = await checkDuplicatePatient(fullName, phoneNumber, exclude_id = patientId);
      if (isDuplicate) {
        hideLoader();
        Swal.fire({
          icon: 'warning',
