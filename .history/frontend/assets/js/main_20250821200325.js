...
// ----------------- Print PDF Functions ------------------- //
// PRINT (preview inside iframe and print)
document.getElementById("printPdfBtn").addEventListener("click", async function () {
  const printBtn = this;
  const container = document.querySelector(".prescription-body"); // your HTML prescription lives here
  const iframe = document.getElementById("pdfPreviewFrame");

  if (!container || !iframe) {
    alert("Prescription preview area or PDF iframe not found.");
    return;
  }

  // If a PDF is already loaded in the iframe, just print it.
  if (iframe.src) {
    try {
      ...
      console.error("Printing existing iframe failed, regenerating...", err);
      // fall through to regenerate
    }
  }

  // Make sure html2canvas ignores the iframe when rendering the HTML
  iframe.setAttribute("data-html2canvas-ignore", "true");

  // Optional: show a lightweight loading state on the button
  ...
  // Build a filename (not strictly needed for print, but harmless)
  const form = document.querySelector("#appointment_no")?.closest("form");
  const inputs = form?.querySelectorAll("input.form-control.bg-light");
  const patientName = inputs && inputs[1] ? inputs[1].value : "Patient";
  const appointmentNumber = document.querySelector("#appointment_no")?.innerText || "Appointment";
  ...
  const opt = {
    ...
    filename: fileName,                            // used only if you later call pdf.save()
    image: { type: "jpeg", quality: 0.98 },
  ...
  try {
    // Generate PDF from the HTML prescription (excluding the iframe)
    const pdf = await html2pdf().set(opt).from(container).toPdf().get("pdf");
    const blob = pdf.output("blob");
    const url = URL.createObjectURL(blob);

    // Load PDF into the iframe (so we can print reliably inside PyWebView)
    iframe.onload = function () {
      try {
        iframe.contentWindow?.focus();
        iframe.contentWindow?.print();
      } finally {
        // Revoke the blob URL after a short delay to let the print dialog open
        setTimeout(() => URL.revokeObjectURL(url), 10000);
      }
    };
    iframe.src = url; // triggers onload → print
  } catch (err) {
    console.error(err);
  ...
  //       const url = pdf.output('bloburl'); // generates a usable blob URL
