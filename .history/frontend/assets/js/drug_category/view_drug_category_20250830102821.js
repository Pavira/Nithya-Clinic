(function () {
  ...
  async function fetchDrugCategories(isInitial = false, pageNumber = 1) {
    const searchValue = document.getElementById("category_search_value").value.trim().toUpperCase();
        ...
        row.innerHTML = `
          ...
          <td>
            ...
            <button class="btn btn-sm btn-outline-danger" title="Delete Category" 
              onclick="deleteDrugCategory('${category.DrugCategoryId}')">
  ...
  async function viewDrugCategoryFunction() {
    const searchInput = document.getElementById("category_search_value");
