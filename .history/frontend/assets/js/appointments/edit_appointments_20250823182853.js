...
$(document).on("keyup", ".select2-search__field", function () {
  ...
  const exists = instructionList.some(item => item.toLowerCase() === inputVal);
...
$("#add-instruction-btn").on("click", async function (e) {
  ...
  try {
    showLoader();
    const token = localStorage.getItem("token");
    const response = await fetch("/api/v1/appointments/add_instruction", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`
      },
      body: JSON.stringify({ descriptions: instructionList })
    });
    ...
    if (result.success) {
      ...
      instructionList.push(newInstruction);
      localStorage.setItem("instruction_list", JSON.stringify(instructionList));
