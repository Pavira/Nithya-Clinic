...
//   });
// }
// Updated function to accept the button
function micfunction(button) {
  const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
  if (!SpeechRecognition) {
    alert("Your browser does not support Speech Recognition.");
    return;
  ...
  recognition.onresult = function (event) {
    ...
    // Insert transcript into Select2 search box
    const searchBox = document.querySelector('.select2-container input.select2-search__field');
    if (searchBox) {
...
function micButtonForDrugfunction() {
...
async function initEditAppointmentsPage() {
  ...
  // Initialize Select2 with tags option to allow custom input
  $('#drug_name').select2({
    // tags: true,
    placeholder: "Select or speak a drug name",
    allowClear: true,
    width: "100%"
  });
