  ...
  async function fetchAppointment(status, date, searchType, searchValue) {
    ...
    try {
      ...
      appointments.forEach((appointment, index) => {
        ...
        // Check if appointment is Closed → show PDF button
        const pdfBtn = appointment.AppointmentStatus === "Closed" ? `
          <button class="btn btn-sm btn-outline-danger ms-2" 
                  onclick="getPrescription('${appointment.AppointmentRegNum}', '${appointment.PatientRegistrationNumber}')" 
          ...
          <button class="btn btn-sm btn-outline-primary" onclick = "loadPage('appointments/edit_appointments', 
                {
                  ...
                  patient_appointment_datetime: '${format_datetime_local}', 
                  patient_appointment_category: '${appointment.AppointmentCategory}', 
                  patient_department: '${appointment.Department}', 
                  patient_appointment_information: '${appointment.AppointmentInformation}', 
                  user:'${appointment.User}',
                  formatted_datetime: '${formate_log_datetime}', 
                  appointment_id: '${appointment.AppointmentRegNum}'
