  ...
  console.log("initEditAppointmentsPage");
    ...
    try {
        ...
        if(appointment_status === "Closed" ) {
          ...
          let localStorageInstructionList = JSON.parse(localStorage.getItem("templates_list")) ?? [];

          if (localStorageInstructionList.length === 0) {
            localStorageInstructionList = await fetchInstructions();
            localStorage.setItem("drug_list", JSON.stringify(localStorageInstructionList));
          }

          populateDrugOptions(localStorageInstructionList);
