...
async function editDrugNameFunction() {
  ...
  const duplicateCheckInput = document.getElementById("drugName");
  const checkduplicate = await checkDuplicateDrugName(duplicateCheckInput.value.trim().toUpperCase());
  if (!checkduplicate) {
    return
  }
