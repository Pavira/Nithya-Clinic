...
// checking duplicate name and phone number while typing
  document.getElementById("drugName").addEventListener("blur", async function () {
    ...
    if (drugName) {  
      showLoader();
      const isDuplicate = await checkkDuplicateDrugName(drugName);
      if (!isDuplicate) {
        hideLoader();
        Swal.fire({
          icon: 'warning',
...
async function checkkDuplicateDrugName(drugName) {
  console.log("Checking for duplicate drug name...");

  try {
    const token = localStorage.getItem("token");
    const url = `/api/v1/patients/check_duplicate_drug_name?drug_name=${encodeURIComponent(drugName)}`;
