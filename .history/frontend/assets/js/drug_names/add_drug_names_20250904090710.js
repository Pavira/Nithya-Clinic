    ...
    if (e.target && e.target.name === "drugName") {
      ...
      if (drugName) {
        ...
        if (duplicateInForm.length > 1) {
          document.getElementById("add-drug-name-btn").disabled = true;
          Swal.fire({
            icon: "warning",
      ...
      document.getElementById("add-drug-name-btn").disabled = false; // Enable if no issues
    }
